...
import { AcaApi, KarApi, KsmApi, RELAY_CHAIN_TOKEN, Logger, PolkaApi } from "../utils";
import { FixedPointNumber } from "@acala-network/sdk-core";

/// check DOT/KSM balance between parachain-account and total-issuance,
/// send [error] message if diff > 1%.
export const relayChainTokenCheck = async (info = false) => {
  let strings = "";
  ...
  if (
    ksmBalance.sub(karBalance).div(karBalance).toNumber() > 0.01 ||
    karBalance.sub(ksmBalance).div(ksmBalance).toNumber() > 0.01 ||
    info
  ) {
  ...
  if (
    acaBalance.sub(polkaBalance).div(polkaBalance).toNumber() > 0.01 ||
    polkaBalance.sub(acaBalance).div(acaBalance).toNumber() > 0.01 ||
    info
  ) {
  ...
  if (strings != "" || info) {
    Logger.pushEvent(RELAY_CHAIN_TOKEN, `%%% \n ${strings} \n %%% @slack-Acala-data-warn-bot`, "normal", info ? "info" : "error");
  }
};
